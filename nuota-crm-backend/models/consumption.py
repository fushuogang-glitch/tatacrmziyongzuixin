...
class PackageConsumption(Base):
    ...
    工单完成时自动写入，记录每次扣费的完整信息
    """
    __tablename__ = "package_consumptions"

    id = Column(Integer, primary_key=True, index=True)
    package_id = Column(Integer, ForeignKey("service_packages.id"), index=True)
    ...
    deducted_amount = Column(Numeric(10, 2))                # 扣费金额
    ...
    assistant_name = Column(String(50))                     # 助理
    rating = Column(Integer)                                # 满意度1-5
