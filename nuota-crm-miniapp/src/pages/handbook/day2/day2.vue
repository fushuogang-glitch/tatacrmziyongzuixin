<template>
  <view class="hb">
    <view class="title">Day2 · 推演与打法</view>
    <view class="card">
      <view class="q">战略要解决的真问题：</view>
      <textarea v-model="data.problem" placeholder="请输入" />
      <view class="q">打法 / 抓手：</view>
      <textarea v-model="data.tactics" placeholder="请输入" />
...
const sid = ref(0);
const data = reactive<any>({ problem: '', tactics: '', homework: '' });
...
onMounted(async () => {
  ...
  if (sid.value) {
    ...
    Object.assign(data, h?.day2_data || {});
  }
});
...
async function save() {
  ...
  try {
    await api.saveHandbook(sid.value, { day2_data: { ...data } });
    uni.showToast({ title: '已保存' });
  } catch (_) {} finally { uni.hideLoading(); }
}
function next() { uni.redirectTo({ url: '/pages/handbook/day3/day3' }); }
</script>
